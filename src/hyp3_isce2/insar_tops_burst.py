...
def insar_tops_burst(
    ...
    insar_roi = get_region_of_interest(
        ref_footprint, sec_footprint, is_ascending=is_ascending
    )
    dem_roi = ref_footprint.intersection(sec_footprint).bounds

    if abs(dem_roi[0] - dem_roi[2]) > 180.0 and dem_roi[0] * dem_roi[2] < 0.0:
        raise ValueError(
            "Products that cross the anti-meridian are not currently supported."
        )
    ...
    dem_path = download_dem_for_isce2(
        dem_roi, dem_name="glo_30", dem_dir=dem_dir, buffer=0, resample_20m=False
    )
    ...
    if apply_water_mask:
        topsapp.run_topsapp(
            start="computeBaselines", end="filter", config_xml=config_path
        )
        ...
        topsapp.run_topsapp(
            start="computeBaselines", end="unwrap2stage", config_xml=config_path
        )
...
def insar_tops_single_burst(
    ...
    multilook_position = multilook_radar_merge_inputs(
        swath_number, rg_looks=range_looks, az_looks=azimuth_looks
    )

    pixel_size = packaging.get_pixel_size(looks)
    product_name = packaging.get_product_name(
        reference, secondary, pixel_spacing=int(pixel_size), slc=False
    )

    product_dir = Path(product_name)
    product_dir.mkdir(parents=True, exist_ok=True)

    packaging.translate_outputs(
        product_name, pixel_size=pixel_size, include_radar=True, use_multilooked=True
    )
...
def main():
    ...
    parser.add_argument(
        "--reference", type=str.split, nargs="+", help='List of reference scenes"'
    )
    parser.add_argument(
        "--secondary", type=str.split, nargs="+", help='List of secondary scenes"'
    )
    parser.add_argument(
        "--looks",
    ...
    if has_granules and has_ref_sec:
        parser.error(
            "Provide either --reference and --secondary, or the positional granules argument, not both."
        )
    elif not has_granules and not has_ref_sec:
        parser.error(
            "Either --reference and --secondary, or the positional granules argument, must be provided."
        )
    elif has_granules:
        warnings.warn(
