...
def test_make_browse_image():
    ...
    assert (
        open(output_png, "rb").read()
        == open("tests/data/test_browse_image.png", "rb").read()
    )
...
def check_correctness_of_resample(mask, lat, lon, geotransform, data_type, outshape):
    ...
    resampled_image = utils.resample_to_radar(
        mask, lat, lon, geotransform, data_type, outshape
    )
...
def resample_with_different_case(
    resample_rows, resample_cols, mask_rows, mask_cols, geotransform
):
    ...
    data_type = np.byte
    return check_correctness_of_resample(
        mask, lat, lon, geotransform, data_type, outshape
    )
...
def test_create_image(tmp_path):
    def _check_create_image(path: str, image_subtype: str = "default"):
        ...
        img_l = utils.create_image(
            path_l, access_mode="load", image_subtype=image_subtype, action="load"
        )
